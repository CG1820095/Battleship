...
def login_account():
    ...
    if not re.search(r'[A-Z]', password) or not re.search(r'\d', password):
        ...
        cursor.execute("SELECT userid FROM account_details WHERE username = '{}' and userpassword = '{}'".format(username, password))
        connection.commit()
        messagebox.showinfo("BATTLESHIPS", "Login success", "Entering game")
        root.quit()
...
#create account
Button = ttk.Button(root, text="Create Account", command=login_account)  
